...
from tests.settings import ALGORITHM, SECRET_KEY
...
class TestDecodeAccessToken:
    def test_decode_access_token(self, non_empty_db_session, basic_user, basic_user_token):
        ...
        actual_user: Optional[db_User] = decode_access_token(token=access_token, session=db_session)
    ...
    def test_decode_token_existing_user_expired_token(self, non_empty_db_session, basic_user_token_expired):
        ...
        user: Optional[db_User] = decode_access_token(token=token, session=db_session)
        assert user is None
