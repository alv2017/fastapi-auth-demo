...
def authenticated_user(token: str = Depends(oauth2_scheme), session: Session = Depends(get_session)) -> db_User:
    auth_user = decode_access_token(token=token, session=session)
    if not auth_user:
        if not auth_user:
...
def authenticated_staff_user(token: str = Depends(oauth2_scheme), session: Session = Depends(get_session)) -> db_User:
...
def authenticated_admin_user(token: str = Depends(oauth2_scheme), session: Session = Depends(get_session)) -> db_User:
